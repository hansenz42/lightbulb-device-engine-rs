...
pub fn check_type(json_data: &Value, valid_type: &str) -> Result<String, DriverError>{
    let device_type = json_data["device_type"].as_str().ok_or(DriverError("json parser: cannot find device_type in config".to_string()))?;
    if device_type!= valid_type {
        return Err(DriverError("json parser: device_type must be dmx_bus".to_string()));
    }
